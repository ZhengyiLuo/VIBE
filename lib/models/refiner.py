...
class REFINER(nn.Module):
    def __init__(
        ...
        refiner_cfg = Config(cfg)
        self.refiner_model, _, _ = get_models(refiner_cfg, iter = -1)
        self.vibe = vibe
        self.vibe.eval()
    ...
    def state_dict(self):   
        return {
            "refiner": self.refiner_model.state_dict(), 
            "vibe": self.vibe.state_dict()
        }

    def load_state_dict(self, input):   
        self.refiner_model.load_state_dict(input['refiner'])
