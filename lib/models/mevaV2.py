...
from zen_renderer.utils.transform_utils import convert_orth_6d_to_mat, convert_orth_6d_to_aa
from kinematic_synthesis.utils.config import Config
from kinematic_synthesis.lib.model import *
from lib.utils.geometry import rotation_matrix_to_angle_axis,  rot6d_to_rotmat, rotmat_to_6d
...
class MEVAV2(nn.Module):
    ...
    def forward(self, input, J_regressor=None):
        # if self.pretrained and os.path.isfile(self.pretrained) and self.first_in_flag:
        #     pretrained_dict = torch.load(self.pretrained)['model']
        #     self.regressor.load_state_dict(pretrained_dict, strict=False)
        #     print(f'=> loaded pretrained model from \'{self.pretrained}\'')
        #     self.first_in_flag = False
        ...
        init_pose = rotmat_to_6d(convert_orth_6d_to_mat(X_r)).reshape(-1, X_r.shape[-1])
        ...
        #### Jank Bypass As Pre-training/
        # pred_shape = smpl_output[-1]['theta'][:,75:]
        # pred_cam = smpl_output[-1]['theta'][:,:3]
        # _, pred_shape, pred_cam = self.regressor.iter_refine(feature, J_regressor=J_regressor, init_pose = init_pose)
        # pred_rotmat = convert_orth_6d_to_mat(init_pose) 
        # smpl_output = self.regressor.smpl_to_kpts(pred_rotmat, pred_shape, pred_cam, J_regressor)
        #### Jank Bypass As Pre-training/
