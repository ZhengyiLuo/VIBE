...
def main(args):
    ...
    if args.video_dir.endswith(".pkl"):
        print("Loading video list")
        all_videos = sorted(pk.load(open(args.video_dir, "rb")))
    else:
        print(f'Loading video directory {args.video_dir}')
        all_videos = sorted(glob.glob(os.path.join(args.video_dir, "*.*")))
    ...
    for video_idx, video_file in enumerate(all_videos, start=1):
        try:
